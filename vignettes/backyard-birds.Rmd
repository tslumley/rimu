...
The default `image` plot is of the table of the variable by itself and shows the number of co-occurences.  With `type="conditonal"`, the plot shows the proportion of each bird (on the y-axis) given that a particular bird (on the x-axis) is present. 
...
These might be clearer if the birds were ordered by frequency of occurrence. Let's try:

```{r}
bird_presence<-mr_infreq(bird_presence)
image(bird_presence)
image(bird_presence, type="conditional")
```